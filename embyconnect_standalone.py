...
# This file is part of Tautulli4Emby.
#
#  Tautulli4Emby is free software: you can redistribute it and/or modify
#  it under the terms of the GNU General Public License as published by
#  the Free Software Foundation, either version 3 of the License, or
#  (at your option) any later version.
#
#  Tautulli4Emby is distributed in the hope that it will be useful,
#  but WITHOUT ANY WARRANTY; without even the implied warranty of
#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#  GNU General Public License for more details.
#
#  You should have received a copy of the GNU General Public License
#  along with Tautulli4Emby.  If not, see <http://www.gnu.org/licenses/>.
...
# Simple helpers for Emby implementation
class EmbyHelpers:
    @staticmethod
    def timestamp():
    ...
    def milliseconds_to_minutes(ms):
        ...
        return int(ms / 60000) if ms else 0

# Simple HTTP handler for Emby API requests
class EmbyHTTPHandler:
    ...
    def make_request(self, uri, request_type='GET', output_format='dict', data=None, return_response=False):
        ...
        try:
            ...
            if response.status_code == 200:
                if output_format == 'dict' or output_format == '':
...
def get_server_friendly_name(emby_connect):
    """Get server friendly name from Emby"""
    logger.info("Tautulli EmbyConnect :: Requesting name from server...")
    server_name = emby_connect.get_server_info().get('ServerName', '')
...
class EmbyConnect(object):
    """
    Retrieve data from Emby Server - Standalone implementation
    """

    def __init__(self, url=None, api_key=None, timeout=30, ssl_verify=True):
        ...
        if url is None or api_key is None:
            try:
                ...
                self.timeout = timeout if timeout != 30 else config.get('timeout', 30)
                self.ssl_verify = ssl_verify if ssl_verify else config.get('ssl_verify', True)
            except ImportError:
                # Fallback to environment variables if env_config not available
                self.url = url or os.getenv('EMBY_URL', 'http://localhost:8096')
                self.api_key = api_key or os.getenv('EMBY_API_KEY', '')
                self.timeout = timeout
                self.ssl_verify = ssl_verify
        else:
            self.url = url
        ...
        self.request_handler = EmbyHTTPHandler(
            base_url=self.url,
    ...
    def terminate_session(self, session_id='', message=''):
        ...
        """
        if not session_id:
            logger.warning("Tautulli EmbyConnect :: Failed to terminate session: Missing session_id")
            return False
    ...
    def transform_session_data(self, emby_session):
        ...
        transformed_session = {
            ...
            'added_at': EmbyHelpers.timestamp() if now_playing.get('DateCreated') else '',
            ...
            'started': EmbyHelpers.timestamp(),  # Set when session is first seen
    ...
    def transform_user_data(self, emby_user):
        ...
        transformed_user = {
            ...
            # Timestamps
            'created_at': EmbyHelpers.timestamp() if emby_user.get('DateCreated') else '',
            'last_seen': EmbyHelpers.timestamp() if emby_user.get('LastLoginDate') else '',
    ...
    def transform_library_data(self, emby_library):
        ...
        transformed_library = {
            ...
            'agent': 'com.emby.agent.none',  # Emby doesn't use agents like Plex
            ...
            # Timestamps
            'created_at': EmbyHelpers.timestamp(),
            'updated_at': EmbyHelpers.timestamp(),
            'last_accessed': EmbyHelpers.timestamp(),
            'last_scanned': EmbyHelpers.timestamp(),
    ...
    def get_current_activity(self, skip_cache=False):
        ...
        try:
            ...
            return {
                ...
                'sessions': active_sessions  # No session masking needed for standalone version
            }
